...
\begin{center}
    \huge Study of a VBF Jet trigger with the Z Boson
\end{center}
\begin{center}
    \large Andrew Evans
\end{center}
...
Vector Boson Fusion (VBF) is a distinctive production mechanism for W, Z, and Higgs bosons at the Large Hadron Collider (LHC).
With the Compact Muon Solenoid (CMS), a trigger for VBF events based on the two outgoing jets formed would allow for an inclusive Higgs production cross section measurement to be performed.
The Z boson provides a very well understood "stand-in" for the Higgs to facilitate the study of a jet trigger.
\section{Theory}
\subsection{Standard Model Theory Time}
Stuff about it
Some equations that might be nice which belong to and were in peters oral.
...
MY STUFF
The CMS Detector is one of two general purpose detectors at the LHC at the European Center for Nuclear Research (CERN).  It was designed to perform high precision measurements of standard model parameters, including finding the Higgs Boson, and discover beyond the standard model physics.
CMS has four main subdetectors used to measure different particles and quantities, starting from the outside in, there are: the tracker, the electromagnetic calorimeter (ECAL), the hadronic calorimeter (HCAL), and the muon chambers.
The solenoid fits between HCAl and the muon chambers.  The magnetic field is returned through iron return yokes spaced in the muon chambers which allows for better muon track reconstruction.
The coordinate system used by CMS has the positive $z$-axis parallel to counterclockwise(looking down) rotating beam, $\phi$ is the azimuthal angle measured from a line pointing to the center of the ring and the pseudorapidity $\eta \equiv - \ln \left[ \tan\left( \theta/2 \right) \right]$ is a measure of the zenith angle $\theta$ straight up.
\begin{figure}[h]
    \begin{center}
